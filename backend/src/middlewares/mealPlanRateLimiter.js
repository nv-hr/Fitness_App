...
const mealPlanLimiter = rateLimit({
  ...
  handler: (req, res) => {
    ...
    return res.status(429).json({
      ...
      error: { message: 'Meal plan generation limit reached. Please try again later.', code: 'RATE_LIMITED', retryAfter: Math.max(retryAfter, 1) },
    });
  },
...
const logDayLimiter = rateLimit({
  ...
  handler: (req, res) => {
    ...
    return res.status(429).json({
      success: false,
      error: { message: 'Logging limit reached. Please try again later.', code: 'RATE_LIMITED', retryAfter: Math.max(retryAfter, 1) },
    });
  },
  standardHeaders: true,
  legacyHeaders: false,
...
export { mealPlanLimiter, regenerateLimiter, logDayLimiter };
export default mealPlanLimiter;
