-- ============================================================
-- PostgreSQL Seed Data for Fitness_App
-- Re-runnable: TRUNCATE before INSERT + ON CONFLICT DO NOTHING
-- Translation of backend/db/init.sql seed data
-- ============================================================
...
ALTER SEQUENCE activities_id_seq RESTART;
ALTER SEQUENCE foods_id_seq RESTART;

-- ============================================================
-- Activities: 35 Indonesian Home Exercises (Phase 05)
-- English names, Indonesian descriptions (Phase 05 content)
-- ============================================================
...
('Jalan cepat di tempat', 'Berjalan cepat di tempat dengan mengangkat lutut tinggi. Gerakan ringan yang cocok untuk pemula.', 15, 80, '["lose_weight", "maintain"]'::jsonb, '[]'::jsonb),
('Jumping jacks', 'Lompat dengan membuka kaki dan mengangkat tangan ke atas secara bersamaan. Gerakan cardio klasik.', 10, 100, '["lose_weight", "maintain"]'::jsonb, '[]'::jsonb),
('High knees', 'Berlari di tempat dengan mengangkat lutut setinggi pinggang. Tingkatkan intensitas secara bertahap.', 10, 110, '["lose_weight"]'::jsonb, '[]'::jsonb),
('Lari di tempat', 'Berlari di tempat dengan langkah cepat dan konsisten. Jaga postur tubuh tetap tegak.', 20, 200, '["lose_weight"]'::jsonb, '[]'::jsonb),
('Skipping tanpa tali', 'Melompat kecil seolah-olah sedang melompati tali. Gerakan ringan untuk cardio.', 10, 90, '["lose_weight", "maintain"]'::jsonb, '[]'::jsonb),
('Dance cardio', 'Menari bebas mengikuti musik dengan gerakan energik. Latihan cardio yang menyenangkan.', 30, 200, '["lose_weight", "maintain"]'::jsonb, '[]'::jsonb),
-- Bodyweight strength (gain_weight focused)
('Push-up', 'Latihan kekuatan dada dan lengan. Posisi tengkurap, dorong tubuh ke atas dengan tangan.', 10, 50, '["lose_weight", "maintain", "gain_weight"]'::jsonb, '[]'::jsonb),
('Squat', 'Berdiri dengan kaki selebar bahu, turunkan tubuh seperti duduk di kursi, lalu berdiri kembali.', 15, 100, '["lose_weight", "maintain", "gain_weight"]'::jsonb, '[]'::jsonb),
('Lunges', 'Langkah maju dengan satu kaki, turunkan lutut belakang hingga hampir menyentuh lantai.', 15, 90, '["lose_weight", "maintain", "gain_weight"]'::jsonb, '[]'::jsonb),
('Plank', 'Tahan posisi tubuh lurus seperti papan dengan bertumpu pada lengan bawah dan ujung kaki.', 10, 40, '["lose_weight", "maintain", "gain_weight"]'::jsonb, '[]'::jsonb),
('Wall sit', 'Sandarkan punggung di dinding dengan posisi duduk imajiner. Tahan selama mungkin.', 10, 60, '["lose_weight", "gain_weight"]'::jsonb, '[]'::jsonb),
('Glute bridge', 'Berbaring telentang, tekuk lutut, angkat pinggul ke atas hingga tubuh membentuk garis lurus.', 15, 70, '["maintain", "gain_weight"]'::jsonb, '[]'::jsonb),
('Tricep dips dengan kursi', 'Duduk di tepi kursi, letakkan tangan di samping pinggul, turunkan dan angkat tubuh dengan lengan.', 10, 50, '["gain_weight"]'::jsonb, '["kursi"]'::jsonb),
('Pike push-up', 'Push-up dengan posisi pinggul terangkat tinggi membentuk huruf V terbalik. Fokus pada bahu.', 10, 55, '["gain_weight"]'::jsonb, '[]'::jsonb),
-- Flexibility/balance (maintain focused)
('Yoga matahari', 'Rangkaian gerakan yoga dasar: mountain pose, forward fold, plank, cobra, dan downward dog.', 20, 80, '["maintain"]'::jsonb, '["matras"]'::jsonb),
('Peregangan leher', 'Miringkan kepala ke kiri dan kanan, tahan 15 detik tiap sisi. Ulangi beberapa kali.', 5, 15, '["maintain"]'::jsonb, '[]'::jsonb),
('Peregangan punggung', 'Duduk bersila, putar tubuh ke kiri dan kanan perlahan. Jaga napas tetap teratur.', 10, 25, '["maintain"]'::jsonb, '[]'::jsonb),
('Cat-cow stretch', 'Posisi merangkak, lengkungkan punggung ke atas (cat) lalu ke bawah (cow) secara bergantian.', 10, 30, '["maintain"]'::jsonb, '["matras"]'::jsonb),
('Child''s pose', 'Duduk bersimpuh, bungkukkan tubuh ke depan dengan tangan menjulur ke depan. Tahan 30 detik.', 10, 20, '["maintain"]'::jsonb, '["matras"]'::jsonb),
('Tree pose', 'Berdiri dengan satu kaki, letakkan telapak kaki lainnya di paha dalam. Jaga keseimbangan.', 10, 30, '["maintain"]'::jsonb, '[]'::jsonb),
-- HIIT circuits (lose_weight/maintain)
('Burpees', 'Dari posisi berdiri, jongkok, letakkan tangan di lantai, lompat kaki ke belakang, push-up, lompat kembali berdiri, dan lompat ke atas.', 15, 150, '["lose_weight"]'::jsonb, '[]'::jsonb),
('Mountain climbers', 'Posisi plank, tarik lutut bergantian ke arah dada dengan cepat seperti mendaki gunung.', 10, 100, '["lose_weight", "maintain"]'::jsonb, '[]'::jsonb),
('Jump squat', 'Lakukan squat biasa, lalu lompat ke atas saat berdiri. Mendarat dengan lembut dan ulangi.', 10, 120, '["lose_weight"]'::jsonb, '[]'::jsonb),
('High plank to down dog', 'Dari posisi plank tinggi, angkat pinggul ke atas membentuk posisi downward dog, lalu kembali plank.', 10, 70, '["lose_weight", "maintain"]'::jsonb, '["matras"]'::jsonb),
-- Core focused (maintain/gain_weight)
('Crunches', 'Berbaring telentang, tekuk lutut, angkat bahu dari lantai dengan mengencangkan perut.', 10, 40, '["maintain", "gain_weight"]'::jsonb, '["matras"]'::jsonb),
('Bicycle crunches', 'Berbaring telentang, angkat kaki, putar tubuh menyentuh siku ke lutut bergantian seperti mengayuh sepeda.', 10, 60, '["lose_weight", "maintain", "gain_weight"]'::jsonb, '["matras"]'::jsonb),
('Russian twist', 'Duduk dengan lutut ditekuk, angkat kaki sedikit dari lantai, putar tubuh ke kiri dan kanan sambil memegang tangan.', 10, 70, '["maintain", "gain_weight"]'::jsonb, '[]'::jsonb),
('Leg raises', 'Berbaring telentang, angkat kedua kaki lurus ke atas hingga 90 derajat, lalu turunkan perlahan.', 10, 50, '["maintain", "gain_weight"]'::jsonb, '["matras"]'::jsonb),
('Side plank', 'Tahan posisi menyamping dengan bertumpu pada satu lengan bawah. Jaga tubuh tetap lurus.', 10, 45, '["maintain", "gain_weight"]'::jsonb, '["matras"]'::jsonb),
-- Additional balanced activities
('Jalan santai pagi', 'Berjalan santai di pagi hari selama 30 menit. Baik untuk kesehatan jantung dan relaksasi.', 30, 120, '["lose_weight", "maintain"]'::jsonb, '[]'::jsonb),
('Senam lantai dasar', 'Rangkaian gerakan senam dasar: rolling, handstand wall-assist, dan bridge. Cocok untuk semua level.', 25, 150, '["maintain", "gain_weight"]'::jsonb, '["matras"]'::jsonb),
('Latihan pernapasan', 'Latihan pernapasan dalam: tarik napas 4 detik, tahan 4 detik, hembuskan 6 detik. Ulangi 10 kali.', 10, 20, '["maintain"]'::jsonb, '[]'::jsonb),
('Stretching seluruh tubuh', 'Peregangan menyeluruh dari kepala hingga kaki. Tahan tiap peregangan 20-30 detik.', 20, 50, '["maintain"]'::jsonb, '[]'::jsonb),
('Latihan keseimbangan satu kaki', 'Berdiri dengan satu kaki, tahan 30 detik, ganti kaki. Tingkatkan durasi secara bertahap.', 10, 30, '["maintain"]'::jsonb, '[]'::jsonb),
('Shadow boxing', 'Tinju bayangan di tempat: jab, cross, hook, uppercut. Gerakan cepat dengan jeda istirahat.', 15, 130, '["lose_weight", "maintain"]'::jsonb, '[]'::jsonb)
ON CONFLICT DO NOTHING;

-- ============================================================
-- Food Ingredients: 201+ International Items (Phase 06)
