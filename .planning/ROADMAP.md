...
- 🚧 **v1.5 Smart Auto-Logging** — Phases 24-29 (in planning)
...
**Goal**: Activity plans persist to database with per-item completed tracking — cache-only plans become durable, survive page refreshes and cache eviction.
**Depends on**: Nothing (new table, independent foundation)
**Requirements**: ACT-01, ACT-03
**Success Criteria** (what must be TRUE):
   1. System creates `activity_plans` table via idempotent SQL migration with columns: `id`, `user_id`, `plan_date`, `plan_data` (JSONB with per-item `logged` flags), `status`, `created_at`, `updated_at`
   2. Generated activity plan is persisted to `activity_plans` table on generation — survives node-cache eviction and page refresh
...
**Requirements**: ACT-02
**Success Criteria** (what must be TRUE):
   1. `POST /api/activity-plans/log-activities` endpoint accepts date and activity item IDs, batch-inserts matching items to `activity_logs` in an atomic transaction
   2. Each inserted row correctly stores `(user_id, activity_date, activity_name, duration_minutes, intensity)` with server-authoritative calorie calculation
   3. Already-logged items (`logged=true` in plan_data) are silently skipped on repeat requests — no duplicate `activity_logs` rows created
   4. `plan_data` JSONB is updated atomically after each batch-log call, marking logged items as `logged: true`
   5. Transaction rolls back fully if any insert fails (explicit BEGIN/COMMIT/ROLLBACK) — no partial commits
**Plans**: TBD

### Phase 26: 3-Day Meal Plan Backend
**Goal**: System generates 3-day meal recommendations (reduced from 7-day) and persists them to a new `daily_meal_plans` table with per-item logged tracking — old `meal_plans` table remains untouched.
**Depends on**: Nothing (new table, independent from activity plan system)
**Requirements**: MEAL-01, MEAL-02
**Success Criteria** (what must be TRUE):
   1. LLM prompt generates a 3-day meal plan (not 7-day) with breakfast, lunch, dinner, snack for each day — prompt is updated from "7 consecutive days" to "3 consecutive days"
   2. System creates `daily_meal_plans` table via idempotent SQL migration with `UNIQUE(user_id, date)` constraint — prevents duplicate entries per user per day
   3. Old `meal_plans` table remains untouched (read-compatible for archive); all new writes go exclusively to `daily_meal_plans`
   4. User can regenerate any single day within the 3-day window without affecting other days — follows existing single-day regeneration pattern
   5. Each daily meal entry in `plan_data` includes per-item `logged: false` flag (default) and server-calculated calorie values
**Plans**: TBD

### Phase 27: Meal Plan Logging
**Goal**: Users batch-log meals from their daily plan to food_logs with single-click per-day or per-meal-type, and logged status persists in plan_data JSONB.
**Depends on**: Phase 26
**Requirements**: MEAL-03, MEAL-04
**Success Criteria** (what must be TRUE):
   1. `POST /api/daily-meal-plans/log-meals` accepts `{ date, mealType? }` and batch-inserts matching unlogged items to `food_logs` — omitting `mealType` logs all meal types for that day
   2. User can log a full day's meals (all 4 meal types) in one click OR log individual meal types (just "breakfast") — both paths work atomically
   3. Each `food_log` row stores server-recalculated calories (`calories_per_100g × portion / 100`) — never trusts LLM-provided calorie values
   4. `plan_data` JSONB is updated to mark logged items (`logged: true`, `logged_at: timestamp`); already-logged items are skipped on repeat requests (idempotent)
   5. Batch insert is wrapped in explicit BEGIN/COMMIT/ROLLBACK — failure of any item rolls back the entire batch, preventing partial food logs
**Plans**: TBD

### Phase 28: Auto-Generation & Inline Management
**Goal**: Plans auto-generate on page visit (no empty state), regenerate is always one click away, rate limits are respected with visible countdown, and one-shot guard prevents infinite regeneration loops.
**Depends on**: Phase 25, Phase 27 (needs logging endpoints for both activity and meal plans)
**Requirements**: AUTO-01, AUTO-02, AUTO-03, AUTO-04
**Success Criteria** (what must be TRUE):
   1. When user visits a plan page and no plan exists for the current period (today for meals, current week for activities), system auto-generates a plan without requiring a manual "Generate" click — auto-gen triggers on page mount with 2s debounce
   2. An always-visible "Regenerate" button is present on both plan sections (not hidden behind hamburger menus or overflow) — clicking it regenerates the full plan for the current period
   3. When the rate limit is exhausted (no remaining quota), auto-gen is blocked server-side and the user sees a countdown timer showing `Xm Ys` until the next available generation — never a generic error message
   4. One-shot guard prevents auto-retry after a failed LLM call — a `useRef` flag (`autoGenAttempted.current`) is set after the first attempt; on failure the user sees a manual "Retry" button instead of an automatic retry
   5. Auto-generation shares the same rate limit bucket as manual generation — the auto-gen endpoint is never rate-limited differently from the manual generate endpoint, preventing quota bypass
**Plans**: TBD

### Phase 29: UI Consolidation
**Goal**: Activity Plan and Meal Plan pages merge into their parent pages (Activities and Food Log) with route redirects — users see plans and logs in one unified view.
**Depends on**: Phase 28 (auto-gen hooks needed before merge)
**Requirements**: UI-01, UI-02, UI-03
**Success Criteria** (what must be TRUE):
   1. Activity Plan section rendered as an inline section on the Activities page (between activity recommendations and the manual activity log form) — no separate `/weekly-plan` page exists
   2. Meal Plan section rendered as an inline section on the Food Log page (between the calorie summary and the food log entries table) — no separate `/meal-plan` page exists
   3. Navigating to `/weekly-plan` automatically redirects to `/activities` via react-router `<Route>` redirect
   4. Navigating to `/meal-plan` automatically redirects to `/food-log` via react-router `<Route>` redirect
   5. Old `/weekly-plan` and `/meal-plan` route entries are removed from the navigation menu and React Router configuration
**Plans**: TBD
**UI hint**: yes
...
*Last updated: 2026-05-31 (v1.5 phases added)*
